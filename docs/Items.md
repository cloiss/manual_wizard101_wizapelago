...
- **Teleport-Friendly**: Use the Social button to teleport to any friendly player. Expected uses are to reach Golem Court via the Drains, Olde Town via the Bazaar, and any of the Three Streets via teleporting to a random person. This is given as a starting item if `friendly_teleports` is enabled, and otherwise is disallowed.

## SpellCard
Every trainable spell card for your primary and secondary school in the accessible areas is an item. By default, you begin with your primary rank 1 spell and must unlock access to use all other spells. 
...
## ItemCard
Every item card available on accessible equipment through shops or the Bazaar is a potential item. By default, you begin with 3-4 rank 1 item cards (to use on your starting wand) and must unlock access to all other item cards. **Note that you must still have access to the appropriate Vendors and Slots to make use of them.**
...
## TreasureCard
A random assortment of treasure card items will act as filler for any leftover space in the pool. Quest rewards, drops, and especially useful treasure cards are more likely to appear. There are about 20 treasure cards in the pool on Recommended settings.

Treasure cards are **single-use** and may be acquired as drops, quest rewards, or bought from the Library or Bazaar (with the appropriate Vendor items). It is recommended to use a text file to track which Treasure Cards you have used.
...
**Note that you can always talk to Vendors for quests such as Pre-Fears, Trade Voyage, or Skullsplitter even if they are locked.**
